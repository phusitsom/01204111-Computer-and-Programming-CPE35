...
def read_data(filename):
    ...
    norm = new_cases / \
        np.max(new_cases, axis=1).reshape((new_cases.shape[0], 1))
...
def max_new_cases_province(DATA, beg_date, end_date):
    ...
    caseByDate = DATA['new_cases'].take(
        indices=range(ind_beg_date, ind_end_date+1), axis=1)
...
def most_similar(DATA, province):
    ...
    sim = list(
        np.sum((DATA['norm_data'][province_ind] - DATA['norm_data'])**2, axis=1))
    return DATA['province_names'][np.argmin(sim[:province_ind] + sim[province_ind+1:])+1]


def most_similar_province_pair(DATA):
    prov = (np.add(DATA['province_names'].astype(object).reshape(-1, 1),
            ' ') + DATA['province_names'].astype(object)).reshape(-1, 1)

    a = np.sum(np.power(np.repeat(
        DATA['norm_data'], 77, axis=0) - np.tile(DATA['norm_data'], (77, 1)), 2), axis=1)
    ...
    if np.sum(same_norm_diff_prov) != 0:
        ...
        ind = (np.take(np.where(a == 0)[0], [np.where(
            (np.mod(np.where(a == 0), 78) != 0))[1:]]))[0][0]
        np.put(b, [ind], np.take(a, [ind]))

    ans = str(np.random.choice(
        np.take(prov, np.where(b == np.nanmin(b)))[0])).split()
