class FakeDataFrame(object):
    ...
    def __to_dict__(self, __file_text:str, __key):
        ...
        if isinstance(__file_text, str):
            ...
            for _row in __file_text.splitlines()[1:]:
                ...
                _row_iter = _row_split[:self.__key_index__] + _row_split[self.__key_index__+1:]
    ...
    def sort(self, by:str, reverse:bool = False, return_as_dict:bool= True):
        ...
        _rtn =  sorted(self.data.items(), key=  lambda x: x[1][by], reverse= reverse)
...
print([fdf.length(),
       ...
       list_str((lambda d: (d[0], d[1]['tvshow']))(fdf.get(max, lambda x: x[1]['tvshow']))),
       ...
       fdf.agg_map('items', sum, lambda x: sum([e[1] for e in sorted(x[1].items(), key = lambda i: i[1], reverse= True)[:2]])/sum(x[1].values()) > 0.7),
