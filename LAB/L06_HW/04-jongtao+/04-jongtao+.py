class JongTao:
    ...
    def addFlightData(self) -> None:
        ...
        self.flightdata.update({inpsplit[0]: dict(zip(
            ['id', 'start', 'end', 'pas_remain', 'pas_max'], [inpsplit[i] for i in [0, 1, 3, 4, 6]]))})
    ...
    def addBookingData(self):
        ...
        if flight['pas_remain'] == flight['pas_max']:
            ...
            self.bookingdata.update(
                {name: {'name': name, 'flight_id': [flight[id]]}})
    ...
    def showBookingData(self):
        ...
        print(
            f"{name} is booking flight code = {' '.join(pas_data['flight_id'])}")
