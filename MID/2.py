...
(lambda scores: print(f"{max(scores)} {sum(scores)/len(scores):.2f} {(sum([(score -(sum(scores)/len(scores)))**2 for score in scores])/len(scores))**0.5:.2f} {min(scores)}"))([int(row['Score']) for row in data])
